...
val appModule = module {
    ...
    // ViewModels
    viewModel { AuthViewModel(authRepository = get(), userPreferencesRepository = get()) }
    viewModel { WalkthroughViewModel() }
    single { UserPreferencesRepository(context = androidContext()) }
    viewModel { MainViewModel(userPreferencesRepository = get(), articleRepository = get()) }
    viewModel { HomeViewModel(articleRepository = get(), userPreferencesRepository = get()) }
}
